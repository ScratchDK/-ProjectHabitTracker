...
class Habit(models.Model):
    ...
    user = models.ForeignKey(
        'users.CustomUser', on_delete=models.SET_NULL, null=True, related_name='habits', verbose_name='Пользователь'
    )
    ...
    periodicity = models.CharField(
        max_length=10, choices=PERIODICITY_CHOICES, default=DAILY, verbose_name='Периодичность'
    )

    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="Active", verbose_name='Статус выполнения'
    )
    ...
    is_public = models.BooleanField(
        default=False, verbose_name='Публичная привычка', help_text='Отметьте, чтобы привычку видели все пользователи'
    )
